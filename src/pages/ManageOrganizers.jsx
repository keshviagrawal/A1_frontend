...
export default function ManageOrganizers() {
    ...
    const handleDisable = async (id, currentStatus) => {
        ...
        try { await api.patch(`/admin/organizers/${id}/disable`); fetchOrganizers(); }
        catch (err) { alert("Action failed"); }
    };

    const handleDelete = async (id) => {
        if (!window.confirm("Are you sure? This will delete the account and profile PERMANENTLY.")) return;
        try { await api.delete(`/admin/organizers/${id}`); setOrganizers(organizers.filter(o => o._id !== id)); }
                    ...
                    <div className="modal-overlay">
                        <div className="modal-content">
                            ...
                            <div className="flex gap-sm justify-end mt">
                                ...
                                <button onClick={handleCreate} className="btn btn-primary">Create Organizer</button>
                            </div>
                        </div>
                    </div>
                )}
